...
describe("USDC", function () {
  ...
  it("initialize USDC", async function () {
    ...
    try {
      const txn = await contract_USDC.mint(addressBob, 1000000);
    ...
    try {
      const txn = await contract_USDC.connect(this.signers.bob).approve(addresscUSDC, 1000000000000000);
      console.log("Transaction hash:", txn.hash);
...

    {
      ...
      let bobBalance = await contract_cUSDC
        .connect(this.signers.bob)
        .balanceOf(tokenBob.publicKey, tokenBob.signature, this.signers.bob.getAddress());

      console.log("Bob's balance: " + fhevmInstance.bob.decrypt(addresscUSDC, bobBalance));

      console.log(
        "total supply: " +
          fhevmInstance.alice.decrypt(
            addresscUSDC,
            await contract_cUSDC.connect(this.signers.alice).totalSupply(tokenAlice.publicKey, tokenAlice.signature),
          ),
      );
    }

    {
      ...
      let carolBalance = await contract_cUSDC
        .connect(this.signers.carol)
        .balanceOf(tokenCarol.publicKey, tokenCarol.signature, this.signers.carol.getAddress());
      console.log("Carol's balance: " + fhevmInstance.carol.decrypt(addresscUSDC, carolBalance));

      try {
        const txn = await contract_cUSDC
          .connect(this.signers.alice)
          .delegateViewerStatus(await this.signers.dave.getAddress(), true);
        console.log("Transaction hash:", txn.hash);
      ...
      let carolBalance2 = await contract_cUSDC
        .connect(this.signers.dave)
        .balanceOf(tokenDave.publicKey, tokenDave.signature, this.signers.carol.getAddress());
      console.log(
        "Carol's balance when dave is trying to fetch it: " + fhevmInstance.dave.decrypt(addresscUSDC, carolBalance2),
      );
    }
